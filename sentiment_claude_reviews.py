        ...
        if response.status_code == 200:
            ...
            if content and len(content) > 0:
                ...
                try:
                    ...
                    st.warning("JSON parsing failed. Attempting to extract information manually.")
                    
                    # Fallback extraction
                    result = {
                        "sentiment_token": "unknown",
                        "sentiment_description": "Could not determine sentiment",
                        "desire_tokens": [],
                        "desire_description": "Could not extract desires",
                        "context_token": "unknown",
                        "context_description": "Could not determine context"
                    }
                    
                    # Try to extract by looking for keys
                    if "sentiment_token" in text_content:
                        pattern = r"sentiment_token[\"']?\s*:\s*[\"']([^\"']+)[\"']"
                        match = re.search(pattern, text_content)
                        if match:
                            result["sentiment_token"] = match.group(1)
                    
                    if "desire_tokens" in text_content:
                        pattern = r"desire_tokens[\"']?\s*:\s*\[(.*?)\]"
                        match = re.search(pattern, text_content, re.DOTALL)
                        if match:
                            tokens_str = match.group(1)
                            tokens = re.findall(r'["\']([^"\']+)["\']', tokens_str)
                            result["desire_tokens"] = tokens
                    
                    if "context_token" in text_content:
                        pattern = r"context_token[\"']?\s*:\s*[\"']([^\"']+)[\"']"
                        match = re.search(pattern, text_content)
                        if match:
                            result["context_token"] = match.group(1)
...
def main():
    ...
    if uploaded_file and api_key:
        ...
        if st.session_state.get('claude_results') is not None:
